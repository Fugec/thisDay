...
test("cached date pages receive the same control classes without a KV rewrite", () => {
  ...
  assert.match(
    seoWorker,
    /const kvKey = `gen-post-v51-[\s\S]*?const normalizedControls = normalizeDatePageCleanLayoutHtml\(\s*ensureEventAnchorNavigationHtml\(\s*normalizeCachedDatePageControlsHtml\(cached, \{ monthName, day \}\),\s*\)/,
  );
