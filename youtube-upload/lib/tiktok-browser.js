...
export async function uploadToTikTok(videoPath, post) {
  ...
  console.log(`  TikTok: launching browser (session ${hasSession ? 'found' : 'not found — login required'})...`);
  ...
  try {
    ...
    // ── Login check ──────────────────────────────────────────────────────────
    const needsLogin = await page.locator(
    ...
    if (needsLogin) {
      ...
      if (username && password) {
        ...
        // Fill in credentials
        await page.locator('input[name="username"], input[placeholder*="email"], input[placeholder*="phone"]')
    ...
    console.log('  TikTok: waiting for upload to finish (may take a few minutes)...');
    await frame.locator('text=Uploading').waitFor({ state: 'hidden', timeout: 300_000 });
    await page.waitForTimeout(2_000);

    // ── Caption ──────────────────────────────────────────────────────────────
    console.log('  TikTok: filling in caption...');
    const captionBox = frame.locator(
    ...
    const postBtn = frame.locator('button:has-text("Post"), button[data-e2e="post-button"]').first();
    await postBtn.waitFor({ timeout: 20_000 });
    await postBtn.click();

    // Wait for success confirmation
    await Promise.race([
      frame.locator('text=Video posted, text=successfully posted').first().waitFor({ timeout: 60_000 }),
      page.waitForURL(u => u.href.includes('manage'), { timeout: 60_000 }),
    ]).catch(() => {
      console.warn('  TikTok: could not auto-confirm success — check browser for result.');
    });
